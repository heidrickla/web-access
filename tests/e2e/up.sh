...
[ -f dc-ca.pem ] || { echo "dc did not come up"; docker logs --tail 40 wa-dc; exit 1; }

for i in $(seq 1 30); do
  if timeout 2 bash -c 'echo > /dev/tcp/127.0.0.1/13389' 2>/dev/null; then echo "rdp up"; break; fi
  sleep 1
done
