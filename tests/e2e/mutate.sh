...
mutate "a sign-in is recorded on a row that reused its id" src/store.rs 's/             WHERE id = ?1 AND incarnation = ?5 AND (sid IS NULL OR sid = ?2)",/             WHERE id = ?1 AND ?5 = ?5 AND (sid IS NULL OR sid = ?2)",/' a_sign_in_never_lands_on_a_row_that_reused_its_id
mutate "a row bound to one account takes another's sign-in" src/store.rs 's/             WHERE id = ?1 AND incarnation = ?5 AND (sid IS NULL OR sid = ?2)",/             WHERE id = ?1 AND incarnation = ?5",/' a_row_bound_by_one_account_refuses_a_sign_in_from_another
mutate "a change from a page loaded before an import is accepted" src/web.rs 's/        if current.is_empty() || sent != Some(current.as_str()) {/        if false {/' a_change_from_a_page_loaded_before_an_import_is_refused
mutate "an import keeps the instance pages carry" src/migrate.rs 's/        crate::app::new_instance(&staged)?;//' an_import_gives_the_database_a_new_instance_even_when_restoring_its_own_export
mutate "group ids are reused" src/store.rs 's/^    id   INTEGER PRIMARY KEY AUTOINCREMENT,$/    id   INTEGER PRIMARY KEY,/' a_deleted_rows_id_is_never_given_to_a_new_row
