...
| Each user and server row carries a random incarnation set on insert. A sign-in records its login and creates its session only on the row it authenticated, by id and incarnation; a connection's end is written only if its user and server rows still carry the incarnations seen at admission | SQLite gives a new row a deleted row's id; neither a sign-in nor a late end lands on the new row |
| The serving process locks `serving.lock` in the data directory before it builds its runtime or opens the database, and releases it after the runtime has shut down and its blocking work has finished. A second serving process on the same data directory is refused; command-line tools do not take the lock | one database has one server, and no second process settles state the first is still using |
